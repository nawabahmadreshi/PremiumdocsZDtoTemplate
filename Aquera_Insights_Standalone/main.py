...
# Documentation Generation routes removed in Standalone Insights project
...
def ingest_tracking_event():
    ...
    try:
        ...
        event = {
            ...
            "city": data.get('city') or request.headers.get('x-vercel-ip-city', ''),
            "region": data.get('region') or request.headers.get('x-vercel-ip-country-region', ''),
            "country": data.get('country') or request.headers.get('x-vercel-ip-country', ''),
            "country_code": data.get('country_code') or request.headers.get('x-vercel-ip-country', ''),
            "india_user": "Yes" if (data.get('country_code') or request.headers.get('x-vercel-ip-country', '')).upper() == 'IN' else "No",
            "log_id": log_id,
            "log_timestamp": ts,
...
def system_info():
    ...
    smtp_configured = bool(
        ...
        (os.environ.get('BACKUP_RECEIVER_EMAIL'))
    )
